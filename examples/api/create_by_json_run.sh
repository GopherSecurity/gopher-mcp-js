...
# Run the TypeScript SDK example for GopherAgent.createWithServerConfig
# against the npm-published @gopher.security/gopher-mcp-js package.
# Bootstraps a fresh node_modules in
# examples/api/test-project-create-by-json/, installs the SDK from
...
WORK_DIR="$SCRIPT_DIR/test-project-create-by-json"
SDK_VERSION="${SDK_VERSION:-latest}"

echo -e "${GREEN}===========================================${NC}"
echo -e "${GREEN}GopherAgent.createWithServerConfig example${NC}"
echo -e "${GREEN}===========================================${NC}"
echo ""
...
{
  "name": "@gopher.security/gopher-mcp-js-create-by-json-example",
  "version": "1.0.0",
  "private": true,
  "scripts": {
    "start": "tsx create_by_json.ts"
  },
  "dependencies": {
...
cp "$SCRIPT_DIR/create_by_json.ts" .
