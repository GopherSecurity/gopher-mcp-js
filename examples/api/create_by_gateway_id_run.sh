...
# Run the TypeScript SDK example for GopherAgent.createWithGatewayId
# against the npm-published @gopher.security/gopher-mcp-js package.
# Bootstraps a fresh node_modules in
# examples/api/test-project-create-by-gateway-id/, installs the SDK
...
WORK_DIR="$SCRIPT_DIR/test-project-create-by-gateway-id"
...
echo -e "${GREEN}=======================================${NC}"
echo -e "${GREEN}GopherAgent.createWithGatewayId example${NC}"
echo -e "${GREEN}=======================================${NC}"
echo ""
...
if [ -z "$GOPHER_MCP_GATEWAY_ID" ]; then
    echo -e "${YELLOW}Warning: GOPHER_MCP_GATEWAY_ID environment variable is not set${NC}"
    echo -e "${YELLOW}Set it with: export GOPHER_MCP_GATEWAY_ID=gw-...${NC}"
    echo ""
fi
...
{
  "name": "@gopher.security/gopher-mcp-js-create-by-gateway-id-example",
  "version": "1.0.0",
  "private": true,
  "scripts": {
    "start": "tsx create_by_gateway_id.ts"
  },
  "dependencies": {
...
cp "$SCRIPT_DIR/create_by_gateway_id.ts" .
