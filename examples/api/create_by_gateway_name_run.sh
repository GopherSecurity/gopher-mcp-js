...
# Run the TypeScript SDK example for GopherAgent.createWithGatewayName
# against the npm-published @gopher.security/gopher-mcp-js package.
# Bootstraps a fresh node_modules in
# examples/api/test-project-create-by-gateway-name/, installs the SDK
...
WORK_DIR="$SCRIPT_DIR/test-project-create-by-gateway-name"
SDK_VERSION="${SDK_VERSION:-latest}"

echo -e "${GREEN}=========================================${NC}"
echo -e "${GREEN}GopherAgent.createWithGatewayName example${NC}"
echo -e "${GREEN}=========================================${NC}"
echo ""
...
if [ -z "$GOPHER_MCP_GATEWAY_NAME" ]; then
    echo -e "${YELLOW}Warning: GOPHER_MCP_GATEWAY_NAME environment variable is not set${NC}"
    echo -e "${YELLOW}Set it with: export GOPHER_MCP_GATEWAY_NAME=my-gateway${NC}"
    echo ""
fi
...
{
  "name": "@gopher.security/gopher-mcp-js-create-by-gateway-name-example",
  "version": "1.0.0",
  "private": true,
  "scripts": {
    "start": "tsx create_by_gateway_name.ts"
  },
  "dependencies": {
...
cp "$SCRIPT_DIR/create_by_gateway_name.ts" .
