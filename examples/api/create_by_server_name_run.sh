...
# Run the TypeScript SDK example for GopherAgent.createWithServerName
# against the npm-published @gopher.security/gopher-mcp-js package.
# Bootstraps a fresh node_modules in
# examples/api/test-project-create-by-server-name/, installs the SDK
...
WORK_DIR="$SCRIPT_DIR/test-project-create-by-server-name"
SDK_VERSION="${SDK_VERSION:-latest}"

echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}GopherAgent.createWithServerName example${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
if [ -z "$GOPHER_MCP_SERVER_NAME" ]; then
    echo -e "${YELLOW}Warning: GOPHER_MCP_SERVER_NAME environment variable is not set${NC}"
    echo -e "${YELLOW}Set it with: export GOPHER_MCP_SERVER_NAME=my-server${NC}"
    echo ""
fi
...
{
  "name": "@gopher.security/gopher-mcp-js-create-by-server-name-example",
  "version": "1.0.0",
  "private": true,
  "scripts": {
    "start": "tsx create_by_server_name.ts"
  },
  "dependencies": {
...
cp "$SCRIPT_DIR/create_by_server_name.ts" .
