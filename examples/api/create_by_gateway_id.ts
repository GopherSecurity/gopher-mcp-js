#!/usr/bin/env npx tsx
/**
 * SDK example for GopherAgent.createWithGatewayId.
 *
 * TypeScript port of gopher-orch/examples/sdk/api/create_by_gateway_id.cc.
 *
 * Scopes a GopherAgent to a single MCP gateway in the caller's workspace.
 * Internally this hits the same GET /v1/mcp-servers endpoint as
 * createWithApiKey under the Bearer api key, but adds the
 ...
 *
 * Usage:
 *   npx tsx create_by_gateway_id.ts                              # built-in query
 *   npx tsx create_by_gateway_id.ts "query one" "query two" ...  # supplied queries
 */

import { GopherAgent } from '../../src';

const API_KEY_PLACEHOLDER = '{YOUR_GOPHER_API_KEY}';
const GATEWAY_ID_PLACEHOLDER = '{YOUR_MCP_GATEWAY_ID}';
...
function main(): void {
  console.log('=== GopherAgent.createWithGatewayId example ===');
  console.log(`Usage: npx tsx ${__filename} [query1] [query2] ...`);
  console.log('Env:   GOPHER_API_KEY GOPHER_MCP_GATEWAY_ID LLM_PROVIDER LLM_MODEL DEBUG');
  console.log('');
  ...
  const gatewayId = envOr('GOPHER_MCP_GATEWAY_ID', GATEWAY_ID_PLACEHOLDER);
  ...
  console.log(
    `MCP gateway id: ${gatewayId === GATEWAY_ID_PLACEHOLDER ? `${gatewayId}  (set GOPHER_MCP_GATEWAY_ID)` : gatewayId}`
  );
  ...
  if (
    ...
    gatewayId === GATEWAY_ID_PLACEHOLDER
  ) {
    console.error('\nError: LLM_MODEL, GOPHER_API_KEY, and GOPHER_MCP_GATEWAY_ID must all be set.');
    process.exit(1);
  }

  console.log('\nCreating agent via GopherAgent.createWithGatewayId...');
  const agent = GopherAgent.createWithGatewayId(
    provider,
    model,
    apiKey,
    gatewayId
